...
describe("localIconUrl", () => {
  ...
  it("serves a stored absolute icon from this origin", () => {
    assert.equal(
      localIconUrl("https://roshingel.site/static/svg/icon/python.svg"),
      "/static/svg/icon/python.svg",
    );
  });

  it("rewrites the www and http forms too", () => {
    assert.equal(localIconUrl("https://www.roshingel.site/static/a.svg"), "/static/a.svg");
    assert.equal(localIconUrl("http://roshingel.site/static/a.svg"), "/static/a.svg");
  });
  ...
  it("rewrites only the static prefix, not the whole domain", () => {
    const other = "https://roshingel.site/media/a.svg";
