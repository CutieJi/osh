...
describe("renderSitemap", () => {
  ...
  it("emits one <url> per entry", () => {
    const xml = renderSitemap([entry(), entry({ url: "https://roshingel.site/about" })]);
  ...
  it("escapes a URL that would otherwise break the document", () => {
    const xml = renderSitemap([entry({ url: "https://roshingel.site/blog?a=1&b=2" })]);
