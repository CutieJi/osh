...
/*
 * `YYYY-MM-DD`, in Manila. Unambiguous, aligns in a column, and reads in the
 * same order as the sort it usually sits under -- and rendered in the admin's
 * one timezone, so a row created at 06:30 PHT does not show yesterday.
 */
describe("adminDate", () => {
  ...
  it("renders in Manila, so a late-evening UTC timestamp is already tomorrow", () => {
    assert.equal(adminDate("2026-01-23T16:30:00Z"), "2026-01-24");
  });
...
describe("adminDateTime", () => {
  it("appends a 24-hour clock", () => {
    assert.equal(adminDateTime("2026-01-23T12:00:00Z"), "2026-01-23 20:00");
  });

  it("uses 24-hour time so the column sorts as it reads", () => {
    assert.equal(adminDateTime("2026-01-23T00:30:00Z"), "2026-01-23 08:30");
    assert.equal(adminDateTime("2026-01-23T13:05:00Z"), "2026-01-23 21:05");
  });
